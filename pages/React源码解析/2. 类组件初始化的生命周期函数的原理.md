...
`componentDidMount` 生命周期函数会在虚拟DOM被挂载后执行，然后根据当前fiber的tag找到`ClassComponent`，然后执行`instance.componentDidMount`

`commitRoot -> commitRootImpl -> commitLayoutEffects -> commitLayoutEffectOnFiber -> instance.componentDidMount`
