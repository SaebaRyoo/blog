...
然后看fiber同步执行阶段`workLoopSync -> performUnitOfWork`。`performUnitOfWork`会循环执行，返回值为下一个fiber任务，直到所有的fiber任务被执行完成后停止。
...
在`beginWork`完成后会执行到`completeUnitOfWork -> completeWork` 然后最后都会根据`workInProgress.tag`进入`HostComponent`分支，通过`createInstance`方法生成宿主元素（div、span等），也就是真实的DOM。然后通过`appendAllChildren`插入DOM。

*这中间还有很多case分支，都已经被我删除*
